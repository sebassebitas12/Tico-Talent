...
export function initUserNav() {
  ...
  if (sidebarFooter && !document.getElementById('notifDropdownBtn')) {
    const NOTIF_BASE_DATA = [
      { id: 1, tipo: 'vacante', titulo: 'Nueva vacante compatible con tu perfil', detalle: 'Intel Costa Rica busca un Desarrollador Full Stack Senior con experiencia en React y Node.js.', tiempo: 'Hace 5 min', leida: false, href: 'vacantes.html' },
      { id: 2, tipo: 'postulacion', titulo: 'Tu postulación fue revisada por el reclutador', detalle: 'Amazon CR actualizó el estado de tu postulación al puesto de Cloud Architect a Revisión Técnica.', tiempo: 'Hace 23 min', leida: false, href: 'postulaciones.html' },
      { id: 3, tipo: 'capacitacion', titulo: 'Nuevo recurso disponible: IA para RRHH', detalle: 'El curso Inteligencia Artificial aplicada al Reclutamiento ya está disponible. 18 horas, gratuito.', tiempo: 'Hace 1 hora', leida: false, href: 'capacitacion.html' },
      { id: 4, tipo: 'sistema', titulo: 'Actualización de la plataforma TicoTalent', detalle: 'Hemos mejorado los algoritmos de compatibilidad de vacantes.', tiempo: 'Hace 2 horas', leida: false, href: 'notificaciones.html' },
      { id: 5, tipo: 'vacante', titulo: '5 nuevas vacantes en tu área de interés', detalle: 'Se publicaron 5 nuevas posiciones en Tecnología y Datos en San José y Heredia.', tiempo: 'Hace 3 horas', leida: false, href: 'vacantes.html' },
      { id: 6, tipo: 'postulacion', titulo: 'Entrevista técnica programada', detalle: 'Tienes una entrevista virtual con HP Costa Rica para el martes a las 10:00 AM.', tiempo: 'Hace 5 horas', leida: true, href: 'entrevistas.html' }
    ];
    ...
    function renderDropdown() {
      ...
      notifWrapper.innerHTML = `
        ...
        <div id="notifDropdown" style="display:none;position:absolute;top:calc(100% + 10px);right:0;width:340px;max-width:calc(100vw - 2rem);background:#ffffff;border:1px solid var(--border-subtle,#e5e7eb);border-radius:14px;box-shadow:0 14px 38px rgba(0,0,0,0.18);z-index:99999;overflow:hidden;">
          ...
          list.slice(0, 6).map(n => `
                <a class="notif-drop-item" data-id="${n.id}" href="${escapeHTML(n.href || 'notificaciones.html')}" style="padding:0.75rem 1rem;border-bottom:1px solid #f8fafc;display:flex;gap:0.75rem;align-items:flex-start;cursor:pointer;transition:background 0.15s;text-decoration:none;color:inherit;${n.leida ? 'opacity:0.65;background:#ffffff;' : 'background:rgba(83,16,104,0.03);border-left:3px solid #531068;'}">
                  <span class="notif-drop-item__icon" style="width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0;background:#f3eef6;color:#531068;">
                    ${n.tipo === 'vacante'
                      ? '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="7" width="18" height="13" rx="2"/><path d="M8 7V5a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><path d="M3 12h18"/></svg>'
                      : n.tipo === 'postulacion'
                        ? '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M8 13h8M8 17h6"/></svg>'
                        : n.tipo === 'capacitacion'
                          ? '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m2 8 10-5 10 5-10 5L2 8Z"/><path d="M6 10.5V15c3 2 9 2 12 0v-4.5"/><path d="M22 8v6"/></svg>'
                          : '<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>'}
                  </span>
      ...
      notifWrapper.querySelectorAll('.notif-drop-item').forEach(item => {
        item.addEventListener('click', (e) => {
          ...
          // El enlace de la notificación es el responsable de abrir
          // la sección/evento correspondiente.
          // No se vuelve a renderizar antes de navegar para no destruir
          // el elemento que acaba de recibir el click.
        });
      });
    }
    ...
    // Close dropdown on page scroll (not on scroll inside the dropdown list itself).
    // We listen on document in capture phase; if the scrolled element is inside
    // the dropdown or IS the dropdown, we let it scroll freely.
    document.addEventListener('scroll', (e) => {
      const drop = document.getElementById('notifDropdown');
      if (!drop || drop.style.display !== 'block') return;
      if (drop === e.target || drop.contains(e.target)) return;
      closeGlobalNotifDropdown();
    }, true);
...
export function mostrarToast(mensaje, tipo = "success", duracion = 3200) {
  ...
  const closeSVG = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>';
  toast.innerHTML = `
    <span class="toast__icon">${iconos[tipo] ?? iconos.info}</span>
    <span class="toast__msg">${escapeHTML(mensaje)}</span>
    <button class="toast__close" type="button" aria-label="Cerrar notificación">${closeSVG}</button>
  `;
