...
import { adaptarTarea, avatarUsuario } from "./adapters.js";
          ...
          <article class="job-card" data-id="${t.id}" style="${t.completada ? "opacity: 0.75; background: var(--surface-subtle);" : ""}">
            ...
            <div class="job-card__footer" style="margin-top: 0.75rem;">
              <div>
                ...
                <span class="task-owner"><img src="${t.responsableFoto || avatarUsuario(`responsable-${t.userId}`)}" alt="Responsable" loading="lazy" referrerpolicy="no-referrer"><span>Responsable #${t.userId}</span></span>
              </div>
...
async function cargarTareas() {
  ...
  try {
    ...
    tareasRaw = (data.todos ?? (Array.isArray(data) ? data : [])).filter((item) => !getDeletedIds("todos").includes(String(item.id)));
  ...
  abrirModal(titulo, formularioHTML(tarea), async () => {
    ...
    try {
      if (id) {
        ...
        const adaptada = adaptarTarea({ ...nueva, ...datos, id: nueva.id });
...
async function eliminarTareaConfirmada(id) {
  ...
  try {
    ...
    saveDeletedRecord("todos", { id });
    tareasAdaptadas = tareasAdaptadas.filter((t) => t.id !== id);
    addNotification({ tipo: "sistema", titulo: "Tarea eliminada", detalle: "La tarea fue retirada del pipeline. Podés deshacer la eliminación desde Centro de Notificaciones.", tiempo: "Ahora", href: "tareas.html", accion: { tipo: "restaurar-entidad", entity: "todos", entityId: id, estado: "pendiente", referencia: "Tarea" } });
