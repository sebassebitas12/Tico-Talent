...
async function cargarEmpresas() {
  ...
  try {
    ...
    empresasRaw = (data.carts ?? (Array.isArray(data) ? data : [])).filter((item) => !getDeletedIds("carts").includes(String(item.id)));
  ...
  abrirModal(titulo, formularioHTML(emp), async () => {
    ...
    try {
      if (id) {
        ...
        const adaptada = adaptarEmpresa({ ...nueva, ...datos, id: nueva.id });
...
async function eliminarEmpresaConfirmada(id) {
  ...
  try {
    ...
    saveDeletedRecord("carts", { id });
    empresasAdaptadas = empresasAdaptadas.filter((e) => e.id !== id);
    addNotification({ tipo: "sistema", titulo: "Empresa eliminada", detalle: "La empresa fue retirada del directorio. Podés deshacer la eliminación desde Centro de Notificaciones.", tiempo: "Ahora", href: "empresas.html", accion: { tipo: "restaurar-entidad", entity: "carts", entityId: id, estado: "pendiente", referencia: "Empresa" } });
